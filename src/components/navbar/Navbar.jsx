...
const Navbar = ({ page = "home", user }) => {
  ...
  return (
    <>
      {/* Logo (tombol buka sidebar di mobile) */}
      <div className="sm:hidden flex flex-row items-center gap-5 bg-white shadow-sm sticky top-0 py-2.5 px-3 w-full z-[50]">
        <button className="hover:bg-slate-300 p-1 rounded-sm" onClick={toggleSidebar}>
          <Icon.BurgerMenu />
        </button>
        ...
        <div
          className={`${isOpen ? "translate-x-0" : "-translate-x-full"} sm:translate-x-0 duration-300 bg-transparent flex sm:flex-col h-[100vh] fixed z-[100] sm:sticky top-0 left-0 w-[100vw] sm:w-fit`}
        ...
        >
          ...
          >
            ...
            <nav className="flex flex-col gap-1">
              <NavLink isActive={page == "dashboard"} text="Dashboard" hrefTo={"/"}>
                <Icon.House />
              </NavLink>
